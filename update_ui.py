...
# Update ScriptWorkerThread to emit progress
worker_replacement = '''class ScriptWorkerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Pattern to catch tqdm progress updates like " 42%|"
            ...
            for line in process.stdout:
                ...
                if line_stripped:
                    ...
                    if match:
                        ...
                        # Only emit non-progress lines to log
...
content = re.sub(r'class ScriptWorkerThread.*?_task_running = False', worker_replacement, content, flags=re.DOTALL)


# Add progress bar to data tab
data_tab_replacement = '''        self.btn_download.clicked.connect(self.on_start_data)
...
content = content.replace('''        self.btn_download.clicked.connect(self.on_start_data)
        ...
        self.data_log_output = QTextEdit()''', data_tab_replacement)


# Add progress bar to training tab
train_tab_replacement = '''        self.btn_train.clicked.connect(self.on_start_training)
...
content = content.replace('''        self.btn_train.clicked.connect(self.on_start_training)
        ...
        self.log_output = QTextEdit()''', train_tab_replacement)


# Add progress bar to backtest tab
bt_tab_replacement = '''        self.btn_run_bt.clicked.connect(self.on_run_backtest)
...
content = content.replace('''        self.btn_run_bt.clicked.connect(self.on_run_backtest)
        ...
        self.bt_log_output = QTextEdit()''', bt_tab_replacement)


# Update signal connections in actions
data_action_repl = '''        self.data_progress.setValue(0)
...
content = content.replace('''        self.data_worker = ScriptWorkerThread(script)
        self.data_worker.update_signal.connect(lambda msg: self.append_log(self.data_log_output, msg))
        self.data_worker.finished_signal.connect(self._on_data_finished)''', data_action_repl)


train_action_repl = '''        self.train_progress.setValue(0)
...
content = content.replace('''        self.train_worker = ScriptWorkerThread(script)
        self.train_worker.update_signal.connect(lambda msg: self.append_log(self.log_output, msg))
        self.train_worker.finished_signal.connect(self._on_training_finished)''', train_action_repl)


bt_action_repl = '''        self.bt_progress.setValue(0)
...
content = content.replace('''        self.bt_worker = ScriptWorkerThread("backtest_engine.py")
        self.bt_worker.update_signal.connect(lambda msg: self.append_log(self.bt_log_output, msg))
        self.bt_worker.finished_signal.connect(self._on_bt_finished)''', bt_action_repl)
