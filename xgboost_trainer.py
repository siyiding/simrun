...
class XGBoostTrainer:
    def __init__(self, data_dir="stock_features_parquet", model_dir="models"):
        ...
        logger.add(os.path.join(self.model_dir, "xgboost_training.log"), rotation="10 MB")
        self.model = None
    ...
    def plot_feature_importance(self, model, feature_cols):
        ...
        # 保存为 CSV 供查阅
        feat_imp_path = os.path.join(self.model_dir, "xgboost_feature_importance.csv")
        ...
        plt.barh(top_15['Feature'][::-1], top_15['Importance'][::-1], color='skyblue')
        plt.xlabel('XGBoost Feature Importance')
        plt.title('Top 15 Most Important Features')
        plt.tight_layout()
        
        plot_path = os.path.join(self.model_dir, "xgboost_feature_importance.png")
        plt.savefig(plot_path)
        plt.close()

    def run(self):
        logger.info("=== 开始 XGBoost 训练流程 ===")
        ...
        with open(os.path.join(self.model_dir, "xgboost_metadata.json"), "w") as f:
            json.dump(metadata, f, indent=4)
            
        logger.info("=== XGBoost 训练流程完成 ===")

if __name__ == "__main__":
    trainer = XGBoostTrainer(data_dir="stock_features_parquet", model_dir="models")
    trainer.run()
